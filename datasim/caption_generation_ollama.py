...
def caprtion_gen(metafiles, port):
    ...
    for idx, metafile in its:
        ...
        # if CORRECT and 'spatialized_caption' in json_data.keys():
        #     continue
        if CORRECT and Path(str(metafile).replace('/metadata/', '/metadata_qwen3-8b/')).exists():
            print(f'[Qwen3-8b] Caption already exists for {metafile}')
            continue
        spatialized_caption = []
        ...
        metafile = str(metafile).replace('/metadata/', '/metadata_qwen3-8b/')
        Path(metafile).parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ports', type=int, nargs='+', default=[12341, 12342, 12345])
