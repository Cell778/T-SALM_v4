...
def process_split(input_base: Path, output_base: Path, split: str, batch_size: int = 2):
    ...
    for i in tqdm(range(num_batches), desc=f"Processing {split}"):
        ...
        if not second_audio_2.exists():
            raise FileNotFoundError(f"Batch {i} 缺少第二个音频的 _2 文件: {second_audio_2}")

        # 本批次的拼接音频顺序：[第二个的_2，第一个的_0] (顺序反转)
        batch_audio_files = [second_audio_2, first_audio_0]

        # 对应的 metadata 也按 [*_2.json, *_0.json] (顺序反转)
        batch_json_files = [
        ...
        # 生成输出文件名
        output_name = f"temporal_negative{output_count:06d}"
...
def main():
    ...
    # 输出路径
    output_base = Path('/Users/cellren/Desktop/datasets/temporal_spatial_audio_text/stClotho_temporal_negative')
