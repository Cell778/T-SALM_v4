...
from .model_utilities import (PatchEmbed, Mlp, DropPath, get_linear_layer)
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, cfg, in_chans=1, spec_size=256, patch_size=4, 
                 ...
                 patch_norm=True, norm_before_mlp='ln'):
        ...
        global ADAPT_CONFIG
        ADAPT_CONFIG = cfg.get('adapt', {})
        
        self.patch_embed = PatchEmbed(
            img_size=self.spec_size, patch_size=self.patch_size, in_chans=self.in_chans, 
            embed_dim=self.embed_dim, norm_layer=self.norm_layer, patch_stride = patch_stride, 
            cfg_adapt=cfg.get('adapt', {}), cfg_fusion=cfg.get('model', {}).get('fusion', {}))
        ...
        self.ST = self.spec_size // (2 ** (len(self.depths) - 1)) \
            // self.patch_stride[1] * self.freq_ratio
    ...
    def forward_features(self, x, longer_list=[]):
        frames_num = x.shape[2]
        x = self.patch_embed(x, longer_list) # (B, N, C)
        if self.ape:
        ...
        x = x.permute(0,1,3,2,4).contiguous().reshape(B, C, c_freq_bin, -1) # (B, C, c_freq_bin, T)

        latent_output = self.avgpool(torch.flatten(x, 2))
        latent_output = torch.flatten(latent_output, 1)

        return {'embedding': latent_output, 'feature_maps': x}
        
    def forward(self, x: torch.Tensor, longer_list=[]):
        """
        Args:
        ...
        x = self.forward_features(x, longer_list)

        return x

    # Used for EINV2
    def forward_patch(self, x, longer_list=[]):
        x = self.reshape_wav2img(x) # (B,C,r*F,T//r)
        x = self.patch_embed(x, longer_list) # (B, N, C)
        if self.ape:
            x = x + self.absolute_pos_embed
